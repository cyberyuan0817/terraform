...
// Create the alb ssl for https listener
module "create_alb_ssl" {
    source = "../ACM"
    host_web_address = var.host_web_address
    zone_id = module.route_ALBtraffic_to_domain.Route53_zoneID
}
...
// Create the ecs auto scaling for the prod
module "Ecs_Autoscaling" {
    source = "../Autoscaling"
    env_prefix = var.prod_env_prefix
    ecs_cluster_name = var.prod_ecs_cluster_name
    ecr_repo_name = var.prod_ecr_repo_name
    ecs_service_name = module.Prod_ecs.ECS_info.name

}

// Create the ecs cloudwatch for the prod auto-scaling
module "Ecs_cloudwatch" {
    source = "../Cloudwatch"
    ecs_scale_up_arn = module.Ecs_Autoscaling.as_scale_up
    ecs_scale_down_arn = module.Ecs_Autoscaling.as_scale_down
    ecs_cluster_name = var.prod_ecs_cluster_name
    ecr_repo_name = var.prod_ecr_repo_name
    env_prefix = var.prod_env_prefix
